...
- 프로젝트: chitchat v1.1.1
- 작성일: 2026-04-29 (v0.1.0b0) → 갱신: 2026-05-08 (v1.1.1)
...
- **[최종 해결] NoCacheMiddleware**: 개발 환경(`frozen=False`)에서 JS/CSS/HTML 응답에 `Cache-Control: no-store, must-revalidate` 헤더를 자동 추가하는 미들웨어를 구현했다. `?v=N` 쿼리 파라미터 방식보다 안정적이며, 프로덕션(PyInstaller 번들)에서는 자동으로 비활성화된다.
- 개발 중에는 브라우저 DevTools의 "Disable cache" 옵션도 병행 활성화한다.
...
## 17. ES6 모듈 순환 import 방지 (v1.1.1)

### 교훈

`chat_session.js`가 `chat_composer.js`에서 `connectWebSocket`을 import하고, `chat_composer.js`가 `chat_session.js`에서 `renderMessageBubble`을 import하는 순환 의존성이 발생했다. ES6 모듈에서 순환 import 시 초기화 시점에 함수가 `undefined`가 되어 전체 모듈 체인이 로드되지 않았다.

### 대책

- **공용 함수 분리**: 양쪽에서 공유하는 함수(`renderMessageBubble`)를 `chat_utils.js`로 분리하여 순환을 끊는다.
- **동적 import fallback**: 분리가 불가능한 경우 `await import('./module.js')`로 지연 로드하여 초기화 시점의 순환을 회피한다.
- **규칙**: spec.md §5.1.2에 "순환 import 방지" 규칙을 명시적으로 추가했다.

---

## 18. Optional Chaining 좌측 할당 불가 (v1.1.1)

### 교훈

`document.getElementById(mid)?.style.display = 'none'` 코드가 **"Invalid left-hand side in assignment"** 에러를 발생시켰다. ES6에서 optional chaining(`?.`) 결과는 읽기 전용이므로, 좌측에 사용하여 값을 할당할 수 없다. `node --check`로는 감지되지 않고 브라우저 런타임에서만 에러가 발생하여 디버깅이 어려웠다.

### 대책

- `if (el) el.style.display = 'none'` 패턴으로 명시적 null 체크 후 할당한다.
- 향후 ESLint 도입 시 `no-unsafe-optional-chaining` 규칙으로 빌드 타임에 감지한다.

---

## 19. 이벤트 위임 전면 전환 시 위임 레벨 전략 (v1.1.1)

### 교훈

인라인 `onclick` 핸들러를 이벤트 위임으로 전환할 때, 동적으로 생성되는 모달은 `container` 레벨 위임으로는 캡처되지 않았다. 모달이 `document.body` 직속에 추가되기 때문이다.

### 대책

- **정적 컨텐츠**: `container.addEventListener('click', ...)` 레벨 위임 사용.
- **동적 모달**: `document.body` 레벨 위임 또는 모달 생성 시점에 개별 리스너 등록.
- **메모리 누수 방지**: 이벤트 위임 사용 시 리스너가 container에 1개만 등록되므로, 동적 요소 추가/삭제 시 리스너 정리가 불필요하다.
