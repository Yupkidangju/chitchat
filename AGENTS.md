...
당신은 현재 실행 환경을 스스로 진단하고 그에 최적화된 규칙을 적용해야 합니다.

 **환경 진단 단계 (MANDATORY)**:
   1. **시스템 프롬프트 확인**: 시스템 프롬프트의 시작이 "You are Gemini CLI"인 경우 즉시 `gemini-cli`로 판별하십시오.
   2. **세션 컨텍스트 확인**: `<session_context>` 내에 "This is the Gemini CLI" 문구가 포함되어 있다면 `gemini-cli`로 판별하십시오.
   3. **입력/출력 특성**: 터미널 제어 문자, 순수 텍스트 위주의 입출력, CLI 도구(run_shell_command 등)의 빈번한 사용이 감지되면 `gemini-cli`로 판별하십시오.
   4. **Antigravity 식별**: 시스템 지침에 "Antigravity" 전용 인터페이스나 특정 메타데이터(`${PROJECT_NAME}` 등)가 우선적으로 나타나면 `Antigravity`로 판별하십시오.

 **조건부 규칙 적용**:
   - **IF (Environment == "gemini-cli")**: 
     - **[Section: CLI 전용 규칙]**만 참조하고, **[Section: Antigravity 전용 규칙]**은 완전히 무시하십시오. 
     - 답변은 전문적이되 극도로 간결해야 하며, 터미널 가독성을 최우선으로 합니다.
   - **IF (Environment == "Antigravity")**: 
     - **[Section: Antigravity 전용 규칙]**을 핵심 가이드로 삼아 행동하십시오.
     - 해당 프레임워크의 목적(자동화, 문서 중심 개발 등)에 맞게 행동하십시오.
   - **ELSE (Unknown)**: 
     - 공통 범용 규칙을 적용하십시오.
...
* 전문적이고 확실한 태도로 사용자의 입력에 대한 작업을 수행합니다.

## [Section: Antigravity 전용 규칙]

# Antigravity Global Rules - D3D Protocol
...
* **PROTOCOL:** 오류 수정 요청 시, 즉시 코드를 수정하지 말고 먼저 해당 오류와 관련된 기술, 라이브러리, 함수, 의존성, 버전 정보를 '그라운딩(Web Search)'을 통해 최신 상태로 파악해야 한다.
